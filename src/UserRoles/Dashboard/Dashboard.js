...
class Dashboard extends React.Component {
    ...
    EditLeave = (e) => {
        e.persist()
        let path = `/user/dashboard/edit/${parseInt(e.target.id)}`;
        this.props.history.push(path);
    }
    EditHoliday = (e) => {
        e.persist()
        let path = `/user/dashboard/editHoliday/${parseInt(e.target.id)}`;
        this.props.history.push(path);
    }

    UserLink = (e) => {
        e.persist()
        let path = `/user/dashboard/editUser/${parseInt(e.target.id)}`;
        this.props.history.push(path);
    }
    ProposalLink = (e,id) => {
        e.persist()
        let path = `/user/dashboard/editProposal/${parseInt(id)}`;
        this.props.history.push(path);
    }
  render() {
                    ...
                    <div className="dashbaord-box light-green">
                        ...
                        <div className="block-list">
                          {Timelog && Timelog.data && Timelog.data.map((time,index) =>{
                              return(
                                <div className="user-list-panel" key={index}>
                                    <div className="user-image" id={time.id} style={{backgroundImage: `url(${`${connection.concat(time.userImage)}`})`}}>
                                    </div>
                    ...
                    <div className="dashbaord-box cyan">
                        ...
                        <div className="block-list">
                            ...
                            <div className="user-list-panel" key={index}>
                                ...
                                <div className="leave-task" style={{color:"green",cursor: "pointer"}}  id={lev.id}>{lev.Name}</div>
                                <div className="leave-task">{lev.reason}</div>
                                <div className="leave-date">{moment(lev.dateFrom).format("DD MMM YYYY")}</div>
                                <div className="aproval-status approved" style={{color:"green",cursor: "pointer"}} id={lev.id}>{lev.status}<link/></div>
                            </div>
                                )})}
                        </div>
                    </div>
                    ...
                    <div className="dashbaord-box red">
                        ...
                        {Dashboard.holiday && Dashboard.holiday.map((free,index) =>{
                            ...
                            <div className="user-list-panel events" key={index}>
                                <div className="event-name" id={free.id} style={{color:"green",cursor: "pointer"}}>{free.name}</div>
                ...
                <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-4">
                    <div className="dashbaord-box darkpink">
                        ...
                        <div className="block-list">
                            ...
                            return(
                                <div className="user-list-panel" key={index}>
                                    <div className="user-image" id={doj.id} style={{cursor: "pointer",backgroundImage: `url(${`${connection.concat(doj.userImage)}`})`}}>
                                    </div>
                                    <div className="user-name" style={{color:"green",cursor: "pointer"}} id={doj.id} >{doj.firstName}</div>
                ...
                <div className="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-4">
                    <div className="dashbaord-box purple">
                        ...
                        {Dashboard.followUpDate && Dashboard.followUpDate.map((free,index) =>{
                                return (
                            <div className="user-list-panel events" key={index}>
                                <div className="event-name"  style={{cursor: "pointer",color:"red"}}>{moment(free.followUpDate).format("DD MMM YYYY")}</div>
                                {free && free.followUpTime ?
                                <div className="user-post" style={{cursor: "pointer"}} ><b>At :-  {free.followUpTime}  Hrs</b> (24 Hour Format)</div>
                                :null}
