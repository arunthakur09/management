...
export const getPerformance = (data) => {
  ...
  return async (dispatch)  => {
    ...
    .then(res => {
      ...
      dispatch(actionTypes.GET_PERF_SUCCESS(res.data));
    })
    .catch((error) =>{
...
export const updatePerformisActiveData = (data) => {
  let url = connection.concat(`/api/PerformanceMatrixMainParameters/${data.id}`);
  return async dispatch => {
    dispatch(actionTypes.UPDATE_PERS_ISACTIVE_INIT());
    return axios
    .patch(url, data, {headers: await getHeaders(true)})
    .then(res => {
      console.log(data, "isActive data Updated successfully");
      dispatch(actionTypes.UPDATE_PERS_ISACTIVE_SUCCESS(data));
    })
    .catch(error => {
      console.log(error, "isActive data Updated error");
      dispatch(actionTypes.UPDATE_PERS_ISACTIVE_ERROR(error));
      if(error.response.status === 401) {
        localStorage.removeItem("token");
        localStorage.removeItem("userRole");
        localStorage.removeItem("roleId");
