...
const SalesReducer = (state = initialState, action) => {
    switch(action.type) {
        ...
        const proposalStatus = action.data.status;

        let getData = state.getData;

        return {
            ...state,
            getData:{
                ...getData,
                data: state.getData.data.map(sale=> {
                    if(sale.id === userId) {
                        if(proposalStatus) sale.status = proposalStatus;
                        else {sale.rating = action.data.rating;
                            toast.success(`You Rated ${action.data.rating} Stars!`, {
                                autoClose: 1000
                              });}
                    }
                    return sale;
